...
def customTopo(protocol):
    ...
    clients = []
    for i in range(4):
        client = net.addHost(f'h{i+2}', ip=f'10.0.0.{i+2+1}/24')  # h2 to h5
    ...
    makeTerm(server, cmd=f"bash -c 'python3 src/start_server.py -H 10.0.0.1 -p 9000 -s src/lib/Server/downloads -r {protocol} -v; exec bash'")
    ...
    for idx in range(2):
        ...
        # sleep(1)
        makeTerm(clients[idx], cmd=f"bash -c 'python3 src/upload.py -H 10.0.0.1 -p 9000 -s {src_file} -n {dest_file} -r {protocol} -v; exec bash'")
    ...
    for idx in range(2, 4):
        ...
        # sleep(1)
        makeTerm(clients[idx], cmd=f"bash -c 'python3 src/download.py -H 10.0.0.1 -p 9000 -d {download_path} -n {remote_filename} -r {protocol} -v; exec bash'")
